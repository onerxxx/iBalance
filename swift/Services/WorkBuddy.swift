// WorkBuddy.swift — CodeBuddy 积分 + 多号签到 + token 自动刷新
import AppKit
import Foundation
...
enum WorkBuddyService {
    ...
    /// 重启 WorkBuddy Desktop（切号与同步共享复用）：走 ProcessUtil.relaunch 通用三件套
    ///（清单实例锁 → 清孤儿 Electron 进程 → open -b + 5s 验证重试），与 TRAE/ZCode 同口径。
    /// 可执行文件路径不再硬编码，由 bundle id 从 Info.plist 解析。
    static func restartWorkBuddy() {
        ProcessUtil.relaunch(bundleId: "com.tencent.workbuddy.mac", label: "WorkBuddy",
                             lockDirs: [wbUserDataDir])
    }
